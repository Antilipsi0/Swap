...
<div id="dokan-store-listing-filter-wrap">
    ...
    <div class="left">
        <div class="item">
            ...
            <button class="dokan-store-list-filter-button dokan-btn dokan-btn-theme">
                <?php esc_html_e( 'Filtre', 'teespace' ); ?>
            </button>
        </div>

        <p class="item store-count">
            <?php
            printf( _n( 'Affichage total des magasins : %d', 'Affichage total des magasins : %d', $number_of_store, 'teespace' ), number_format_i18n( $number_of_store ) );
            ?>
        </p>
    </div>
    ...
    <div class="right">
        ...
        <form name="stores_sorting" class="sort-by item" method="get">
            <label><?php esc_html_e( 'Trier par', 'teespace' ); ?>:</label>

            <select name="stores_orderby" id="stores_orderby" aria-label="<?php esc_html_e( 'Trier par', 'teespace' ); ?>">
                <?php
        ...
        <div class="toggle-view item">
            <span class="icons dashicons-screenoptions" data-view="grid-view"></span>
...
<form role="store-list-filter" name="dokan_store_lists_filter_form" id="dokan-store-listing-filter-form-wrap" style="display: none">
    ...
    <?php
        ...
        if ( apply_filters( 'dokan_load_store_lists_filter_search_bar', true ) ) : ?>
            <div class="store-search grid-item">
                <input type="search" class="store-search-input" name="dokan_seller_search" placeholder="<?php esc_html_e( 'Recherche vendeur', 'teespace' ); ?>">
            </div>
        <?php endif;
    ...
    <div class="apply-filter">
        <button id="cancel-filter-btn" class="dokan-btn dokan-btn-theme"><?php esc_html_e( 'Fermer', 'teespace' ); ?></button>
        <button id="apply-filter-btn" class="dokan-btn dokan-btn-theme" type="submit"><?php esc_html_e( 'Appliquer', 'teespace' ); ?></button>
    </div>
